...
class GAT(nn.Module):
	def __init__(self, nfeat, nhid, nlayers=2, heads=8, output_head=1, dropout=0., save_mem=True,
		...
		if with_bn > 0:
			...
			if with_bn == 1:
				...
				self.bns.append(ASRNormBN(nhid*heads))
		...
		self.layers.append(GATConv(nhid*heads, nhid, heads=output_head, concat=False, dropout=dropout))
		...
		self.name = f'{nlayers} layers {heads} heads GAT'
